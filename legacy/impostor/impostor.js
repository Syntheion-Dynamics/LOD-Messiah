/**
 * LEGACY — AABB box / octahedral impostor baker (kept intact, not used by default cook).
 * Prefer LOD3 boxcards (`src/lod3-silhouette.js`). Opt-in via `--impostor`.
 */
import {
  mkdirSync,
  writeFileSync,
...
async function runBlenderImpostor({
  ...
  const script = join(ROOT, 'legacy', 'impostor', 'blender_impostor.py');
  const args = [
    '--background',
    '--python',
    script,
